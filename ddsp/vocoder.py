    ...
    def extract(
        ...
        # extract f0 using rmvpe
            ...
            f0 = self.rmvpe.infer_from_audio(
                audio, self.sample_rate, device=device, thred=0.03, use_viterbi=False
            )
            uv = f0 == 0
            if len(f0[~uv]) > 0:
...
class Volume_Extractor:
    ...
    def extract(self, audio):  # audio: 1d numpy array
        ...
        audio = np.pad(
            audio,
            (int(self.win_size // 2), int((self.win_size + 1) // 2)),
            mode="reflect",
        )
        audio2 = audio**2
        mean = np.array(
            [
                np.mean(
                    audio[
                        int(n * self.hop_size) : int(n * self.hop_size + self.win_size)
                    ]
                )
...
class Units_Encoder:
    def __init__(
        ...
        if encoder == "cnhubertsoftfish":
            self.model = CNHubertSoftFish(
                encoder_ckpt, device=device, gate_size=cnhubertsoft_gate
            )
            is_loaded_encoder = True
    ...
    def encode(
        ...
        ratio = (hop_size / sample_rate) / (
            self.encoder_hop_size / self.encoder_sample_rate
        )
        index = torch.clamp(
            torch.round(ratio * torch.arange(n_frames).to(self.device)).long(),
            max=units.size(1) - 1,
        )
        units_aligned = torch.gather(
            units, 1, index.unsqueeze(0).unsqueeze(-1).repeat([1, 1, units.size(-1)])
        )
...
class Audio2ContentVec:
    def __init__(self, path, h_sample_rate=16000, h_hop_size=320, device="cpu"):
        ...
        print(" [Loading] " + path)
        self.models, self.saved_cfg, self.task = (
...
class Audio2ContentVec768L12TTA2X:
    ...
    def __call__(self, audio):  # B, T
        ...
        with torch.no_grad():
            ...
            feats_tta = torch.cat((feats2, feats), dim=2).reshape(
...
class CNHubertSoftFish(torch.nn.Module):
    def __init__(
        self, path, h_sample_rate=16000, h_hop_size=320, device="cpu", gate_size=10
    ):
        super().__init__()
        self.device = device
        ...
        self.proj = torch.nn.Sequential(
            torch.nn.Dropout(0.1), torch.nn.Linear(768, 256)
        )
...
class CombSubSuperFast(torch.nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        noise_filter = (
            torch.exp(ctrls["noise_magnitude"] + 1.0j * np.pi * ctrls["noise_phase"])
            / 128
        )
