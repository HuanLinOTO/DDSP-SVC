...
class AudioDataset(Dataset):
    def __init__(
        ...
        #     os.path.join(self.path_root, "pitch_aug_dict.npy"), allow_pickle=True
        # ).item()
        if load_all_data:
        ...
        for name_ext in tqdm(self.paths, total=len(self.paths)):
            path_feature1 = os.path.join(self.path_root, "features1", name_ext) + ".npy"
            features1 = np.load(path_feature1, allow_pickle=True).item()

            f0 = features1["f0"]
            volume = features1["volume"]
            aug_vol = features1["aug_vol"]
            frame_len = features1["frame_len"]
            ...
            if load_all_data:
                path_features2 = (
                    os.path.join(self.path_root, "features2", name_ext) + ".npy"
                )
                features2 = np.load(path_features2, allow_pickle=True).item()
                mel = features2["mel"]
                aug_mel = features2["aug_mel"]
                units = features2["units"]
    ...
    def get_data(self, name_ext, data_buffer):
        ...
        start_frame = (
            0
            if self.whole_audio
            else random.randint(0, data_buffer["frame_len"] - self.crop_len)
        )
        units_frame_len = (
            data_buffer["frame_len"] if self.whole_audio else self.crop_len
        )
        ...
        aug_shift = 0
        if aug_flag:
            aug_shift = data_buffer.get("pitch_aug")
        f0_frames = (
